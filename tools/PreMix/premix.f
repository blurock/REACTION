...
C     1. Upgrade to CKLIB.40
C     CHANGES FOR VERSION 2.55 (3/15/94 F. Rupley)
...
C     CHANGES FOR VERSION 2.55b (7/14/94 F. Rupley)
C     1. Add regridding option
      END
C
C/////////////////////////////////////////////////////////////////////
C
...
C
      ...
      CHARACTER C(*)*(*)
C
      COMMON /FLFLFL/ NCKW, NMCW, NEPS, NWT,  NRE, NSCH, NX, NCON, NREG,
     1                NTGV, NXGV, ND, NDKJ, NTDR, NYV, NSCL, NABV, NBLW, 
     2                NBUF, NTWP, NS, NSN, NF, NFN, NDS, NSSV, NKA6, NA, 
     3                ICKW, IMCW, IKS, ICC, IKR, IKI, IKP, IIP, LAC, LMK
C
...
C
   ...
   15 FORMAT (
     ...
     2/'          CHEMKIN-II Version 2.55b, July 1994',
C*****precision > double
     3/'          DOUBLE PRECISION')
C*****END precision > double
C*****precision > single
...
C
     ...
     2             R(NRE), R(NSCH), R(NX), R(NCON), R(NREG), R(NTGV), 
...
C 
     ...
     2                   RMCWRK, EPS, WT, REAC, SCRCHK, X, COND, REG, 
...
C
      CALL RDKEY (KK, NMAX, LIN, LOUT, KSYM, PATM, LBURNR, LTIME,
...
C
  ...
  350       CONTINUE
         ...
         ENDIF
C
         IF (LREGRD .AND. (RSTCNT.OR.LRSTRT) .AND. (JJ.GT.JJREGD))
     1   CALL PRREGD (SN, S, NATJ, JJREGD, JJ, REG, X, COND, PCTADP,
     2                RATGTC, NT, SPOS, KK, NYS, NTEMP, XGIVEN,
     3                TGIVEN, LOUT, LBURNR, TFIXT, SCRCHK(1,4))
         CALL RESTRT (KK, NMAX, NATJ, JJ, LOUT, LMOLE, LBURNR,
     1                LUSTGV, NREAC, REAC, FLRT, NTEMP, XGIVEN,
     3                TGIVEN, XSTR, XEND, ICKWRK, RCKWRK,
     4                EPS, JFIXT, TFIXT, X, S)
C
      ELSE
C
...
C
     ...
     1                    LTDIF, LTIME, JFIXT, TFIXT, FLRT, P,
     2                    WT, EPS, DT, NTEMP, XGIVEN, TGIVEN, X, SN,
...
C
     ...
     1                  LVCOR, LTDIF, LTIME, JFIXT, TFIXT, FLRT,
...
C
     ...
     1        UPPER
      LOGICAL LBURNR, LENRGY, LTDIF, LTIME, LMULTI, LVCOR
C
...
C
     ...
     4               AREAJ * (SUM + TDOT) / CPB
           IF (WNDFAC .EQ. 1.0) THEN
             F(NT,J) = XMDOT * ( S(NT,J) - S(NT,J-1) ) / DXM - ATERM
           ELSE
             F(NT,J) = XMDOT * (CENDFP*S(NT,J+1) + CENDFC*S(NT,J) +
     1                          CENDFM*S(NT,J-1) ) - ATERM
           ENDIF
C
        ELSE
           CALL TEMP (NTEMP, X(J), XGIVEN, TGIVEN, TI)
...
C--------------------------------------------------------------------
C
      SUBROUTINE RDKEY (KK, NMAX, LIN, LOUT, KSYM, PATM, LBURNR,
...
C
     ...
     1             LTIME, JFIXT, TFIXT, FLRT, P, WT, EPS, DT,
...
110   CONTINUE
      ...
      END
C
      SUBROUTINE PRREGD (SN, S, NATJ, JJREGD, JJ, REG, X, COND, PCTADP,
     1                   RATGTC, NT, SPOS, KK, NYS, NTEMP, 
     2                   XGIVEN, TGIVEN, LOUT, LBURNR, TFIXT, SCRK)
C
C*****precision > double
      IMPLICIT DOUBLE PRECISION (A-H, O-Z), INTEGER (I-N)
C*****END precision > double
...
C
      ...
      DIMENSION SN(NATJ,*), S(NATJ,*), REG(*), X(*), COND(*), XGIVEN(*),
     1          TGIVEN(*), SCRK(*)
C
      WRITE (LOUT, *)
