...
C     Sandia National Laboratories Report SAND92-8225, Livermore,
C     California, September 1992.
C
C///////////////////////////////////////////////////////////////////////
C
C     CHANGES FROM THE PREVIOUS VERSION:
C
C     1) REMOVE UNUSED ARRAY "CODE".
C
C     2) CORRECT THE SORTING OF CHANGE BLOCK NAMES WHEREBY THE WRONG
C        TYPE COULD BE ASSOCIATED WITH A NAME, MAKING THE NAME
C        UNRECOGNIZED.
C
C     3) ADD PRINTING OF THE UNRECOGNIZED NAME TO THE ERROR MESSAGE
C        9501.
C
C     4) CORRECT ERROR 9501 TO COPY THE UNRECOGNIZED LINE.
C
C///////////////////////////////////////////////////////////////////////
      ...
      CHARACTER
     +   FILE1*80, FILE2*80, ITEM*8, LIST*80, MARK*8, REMARK*6, SMARK*8
C     THE MICROWAY NDP FORTRAN COMPILER VERSION 4.0.2 PASSES CHARACTER
C     STRINGS INCORRECTLY WITH THE FOLLOWING STATEMENT.
C     EXTERNAL EXTENT, SQUEEZ
      INTEGER
     +   CMND, CMNTS, COUNT, INDEX, ITEMP, J, K, LEN1, LEN2, LEN3, LEN4,
     +   LEN5, LENGTH, LINES, LONG, MAXL, MAXN, MAXS, NAMES, NUMBER,
     +   REPORT, SAVLEN, SAVNUM, STATUS, TOPNUM, TYPE, UNIT1, UNIT2
      LOGICAL
     ...
     +   OPEN, SAVED, SPECL, VARY
      ...
      PARAMETER (MAXS = 200)
C     THE FOLLOWING CHARACTER STRINGS SHOULD HAVE LENGTH EQUAL TO MAXS
      CHARACTER
     +   LINE*200, NAME*200, SAVLIN*200, TEMP*200, TEMP2*200, TEMP3*200,
     +   TOP*200
...
C///  TREAT SPECIAL LINES FOUND FROM INSIDE.
     ...
     +          LINE (10 : 10) .EQ. ' ')
            ...
            IF (NEW) THEN
               ...
               NEW = .FALSE.
            ELSE
               ERROR = ACTIVE(INDEX, 1) .NEQV. FLAG
               IF (ERROR) GO TO 9208
            END IF
...
10001 FORMAT
     +   (/ '    CHANGE:  VERSION 2.04 OF AUGUST 1993',
     +   ' BY JOSEPH GRCAR.')
