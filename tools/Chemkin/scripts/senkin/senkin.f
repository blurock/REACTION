...
C
   ...
   15 FORMAT(
     ...
     2/'          Author: Andy Lutz',
     3/'          CHEMKIN-II Version 3.0, March 1994',
C*****precision > double
     4/'          DOUBLE PRECISION')
C*****END precision > double
C*****precision > single
...
C          GO TO MAIN LEVEL
C
      CALL BEGIN (NSYS, ICASE, II, KK, LENI, LENR, LENC,
...
C---------------------------------------------------------------
C
      SUBROUTINE BEGIN (NSYS, ICASE, II, KK, LENICK, LENRCK,
...
C
     ...
     3      'the number from the CHEMKIN binary file.',
     4      'No. species in restart file = ', KKR,
     5      'No. species in binary file = ', KK
C
...
C
1000  CONTINUE
      IF (TLASTP .NE. TIM) CALL TEXT13 (IPAR, KK, KSYM, LIGN,
...
C
      WRITE (LOUT, '(1X,A,1PE11.4,3X,A,1PE11.4)')
     1 'Time (sec) = ', TIM, 'T (K) = ', Z(1)
C
      PA = P / PATM
