...
packages() {
  PACKAGES_PWD=`pwd`; cd $Sturm/src; ls -d *; cd $PACKAGES_PWD
}

 
#F delfiles . . .  Lists a package's files to be deleted from the STURM tree.
...
delfiles() {
  ...
  find examples/$PACKAGE src/$PACKAGE tools/$PACKAGE -type f \
    ...
    -print
  find include/Sturm -type f \( -name "$PACKAGE".h -o -name "$PACKAGE"Type.hh -o -name "$PACKAGE".hh -o -name "$PACKAGE".irq \) -print
...
files() {
  ...
  find examples src tools -type f \
    \( -name '*.[ch]' -o -name '*.cc' -o -name '*.hh' -o -name '*.icc' -o \
       ...
       -name '*.irq' -o -name Makefile \) \
    -print
  find include/Sturm -type f \( -name '*.h' -o -name '*.hh' -o -name '*.irq' \) -print
